...
def run_command_safe(command, cwd=None, **kwargs):
    """Safe wrapper for subprocess calls that handles packaging"""
...
Your configuration has been saved and Ogresync is ready to use!"""
        ...
        try:
            ...
            if error:
                ...
                if path_error:
                    ...
                    try:
                        result = run_command_safe(['git', '--version'], capture_output=True, text=True)
        ...
        try:
            ...
            if not is_git_repo[0]:  # Not a git repo
                ...
                if error:
                    ...
                    try:
                        run_command_safe(['git', 'init'], cwd=vault_path, check=True)
                        run_command_safe(['git', 'branch', '-M', 'main'], cwd=vault_path, check=True)
            ...
            if os.path.exists(vault_path):
                ...
                try:
                    result = run_command_safe(['git', 'log', '--oneline'], 
            ...
            if has_existing_files or has_existing_git_history:
                ...
                try:
                    run_command_safe(['git', 'add', '-A'], cwd=vault_path, check=True)
                    result = run_command_safe(['git', 'commit', '-m', 'Initial commit with existing vault files'], 
                                          cwd=vault_path, capture_output=True, text=True)
                    if result.returncode == 0:
                ...
                if os.path.exists(readme_path):
                    ...
                    try:
                        run_command_safe(['git', 'add', 'README.md'], cwd=vault_path, check=True)
                        result = run_command_safe(['git', 'commit', '-m', 'Add existing README'], 
                                              cwd=vault_path, capture_output=True, text=True)
                        if result.returncode == 0:
                ...
                try:
                    run_command_safe(['git', 'add', '-A'], cwd=vault_path, check=True)
                    result = run_command_safe(['git', 'commit', '-m', 'Initial commit with README'], 
                                          cwd=vault_path, capture_output=True, text=True)
                    if result.returncode == 0:
                ...
                if email and email.strip():
                    ...
                    try:
                        ...
                        if "timeout" in error_str.lower():
                            ...
                            return False, f"SSH key generation failed: {error_str}"
                else:
        ...
        try:
            ...
            if error:
                ...
                try:
                    ...
                    run_command_safe(['ssh-keyscan', '-H', 'github.com'], 
    ...
    def _step_github_repository(self):
        ...
        try:
            ...
            git_check = run_command_safe(['git', 'status'], cwd=vault_path, capture_output=True, text=True)
            if git_check.returncode != 0:
                self._update_status("Initializing git repository...")
                # Initialize git if not already done
                init_result = run_command_safe(['git', 'init'], cwd=vault_path, capture_output=True, text=True)
                if init_result.returncode != 0:
                    return False, f"Failed to initialize git repository: {init_result.stderr}"
                
                # Set default branch to main
                run_command_safe(['git', 'branch', '-M', 'main'], cwd=vault_path, capture_output=True, text=True)
    ...
    def _step_repository_sync(self):
        ...
        try:
            ...
            if not remote_url:
                ...
                try:
                    result = run_command_safe(['git', 'remote', 'get-url', 'origin'], 
                                          cwd=vault_path, capture_output=True, text=True)
                    if result.returncode == 0:
    ...
    def _get_current_branch(self, vault_path):
        ...
        try:
            ...
            default_branch_result = run_command_safe(['git', 'symbolic-ref', 'refs/remotes/origin/HEAD'], 
                                                 cwd=vault_path, capture_output=True, text=True)
            ...
            remote_branches_result = run_command_safe(['git', 'branch', '-r'], 
                                                  cwd=vault_path, capture_output=True, text=True)
    ...
    def _check_remote_repository(self, vault_path):
        ...
        try:
            ...
            fetch_result = run_command_safe(['git', 'fetch', 'origin'], 
            ...
            if fetch_result.returncode == 0:
                ...
                ls_result = run_command_safe(['git', 'ls-tree', '-r', '--name-only', remote_branch_ref], 
                                         cwd=vault_path, capture_output=True, text=True)
...
"""
            try:
                ...
                # Commit the README
                run_command_safe(['git', 'add', 'README.md'], cwd=vault_path, check=True)
                run_command_safe(['git', 'commit', '-m', 'Initial commit: Add README'], cwd=vault_path, check=True)
            except Exception as e:
    ...
    def _handle_simple_pull(self, vault_path, remote_files, current_step):
        ...
        try:
            ...
            pull_result = run_command_safe(['git', 'pull', 'origin', current_branch, '--allow-unrelated-histories'], 
            ...
            fetch_result = run_command_safe(['git', 'fetch', 'origin', current_branch], 
                                        cwd=vault_path, capture_output=True, text=True)
            ...
            status_before_reset = run_command_safe(['git', 'status', '--porcelain'], 
                                               cwd=vault_path, capture_output=True, text=True)
            ...
            if status_before_reset.stdout.strip():
                ...
                safety_commit_result = run_command_safe(['git', 'add', '-A'], cwd=vault_path, capture_output=True, text=True)
                if safety_commit_result.returncode == 0:
                    commit_msg = f"Safety backup before reset - {time.strftime('%Y-%m-%d %H:%M:%S')}"
                    commit_result = run_command_safe(['git', 'commit', '-m', commit_msg], 
                                                 cwd=vault_path, capture_output=True, text=True)
            ...
            local_commit_result = run_command_safe(['git', 'rev-parse', 'HEAD'], 
                                               cwd=vault_path, capture_output=True, text=True)
            remote_commit_result = run_command_safe(['git', 'rev-parse', remote_branch_ref], 
                                                cwd=vault_path, capture_output=True, text=True)
            
            local_ahead_check = run_command_safe(['git', 'rev-list', '--count', f'{remote_branch_ref}..HEAD'], 
                                             cwd=vault_path, capture_output=True, text=True)
            ...
            checkout_result = run_command_safe(['git', 'checkout', remote_branch_ref, '--', '.'], 
                                           cwd=vault_path, capture_output=True, text=True)
            ...
            reset_result = run_command_safe(['git', 'reset', '--hard', remote_branch_ref], 
                                        cwd=vault_path, capture_output=True, text=True)
    ...
    def _handle_remote_empty(self, vault_path, local_files, current_step):
        ...
        try:
            ...
            if status_result.returncode == 0 and status_result.stdout.strip():
                ...
                # Add all files
                add_result = run_command_safe(['git', 'add', '.'], 
                                          cwd=vault_path, capture_output=True, text=True)
                
                if add_result.returncode == 0:
                    # Commit changes
                    commit_result = run_command_safe(['git', 'commit', '-m', f'Initial vault content - {len(local_files)} files'], 
                                                 cwd=vault_path, capture_output=True, text=True)
        ...
        try:
            ...
            if not conflict_analysis.has_conflicts:
                ...
                try:
                    ...
                    result = run_command_safe(['git', 'pull', 'origin'], 
                                          cwd=vault_path, capture_output=True, text=True, timeout=30)
            ...
            if not conflict_analysis.has_conflicts:
                ...
                try:
                    ...
                    pull_result = run_command_safe(['git', 'pull', 'origin'], 
                                              cwd=vault_path, capture_output=True, text=True, timeout=30)
                    ...
                    if pull_result.returncode == 0:
                        ...
                        merge_result = run_command_safe(['git', 'merge', remote_branch_ref, '--allow-unrelated-histories', '--no-edit'], 
                                                   cwd=vault_path, capture_output=True, text=True, timeout=60)
    ...
    def _handle_simple_merge_fallback(self, vault_path, current_step):
        ...
        try:
            ...
            pull_result = run_command_safe(['git', 'pull', 'origin', current_branch, '--allow-unrelated-histories', '--no-rebase'], 
                                       cwd=vault_path, capture_output=True, text=True, timeout=60)
    ...
    def _handle_unknown_scenario(self, vault_path, current_step):
        ...
        try:
            ...
            if fetch_result.returncode == 0:
                ...
                pull_result = run_command_safe(['git', 'pull', 'origin', 'main', '--allow-unrelated-histories'], 
                                           cwd=vault_path, capture_output=True, text=True, timeout=60)
    ...
    def _step_final_sync(self):
        ...
        try:
            ...
            if status_result.returncode == 0 and status_result.stdout.strip():
                ...
                run_command_safe(['git', 'add', '.'], cwd=vault_path, capture_output=True, text=True)
                commit_result = run_command_safe([
            ...
            if remote_branch_exists.returncode != 0:
                ...
                push_result = run_command_safe(['git', 'push', '-u', 'origin', current_branch], 
    ...
    def _safe_pull_remote_files(self, vault_path, force_reset=False):
        ...
        try:
            ...
            if force_reset:
                ...
                merge_result = run_command_safe(['git', 'merge', remote_branch_ref, '--allow-unrelated-histories', '--no-ff'], 
                                            cwd=vault_path, capture_output=True, text=True)
